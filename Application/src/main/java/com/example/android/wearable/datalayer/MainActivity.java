...
public class MainActivity extends Activity implements DataApi.DataListener,
    ...
    public void onMessageReceived(final MessageEvent messageEvent) {
        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                mDataItemListAdapter.add(new Event("Sensor Data From Watch", wearable_str+mobile_str));
            }
        });
    ...
    {
        final Button btn_start=(Button)findViewById(R.id.btn_startSave);
        final Button btn_startSensor=(Button)findViewById(R.id.btn_startSensor);
        ...
        btn_start.setOnClickListener(new Button.OnClickListener(){
            ...
            public void onClick(View v) {
                ...
                {
                    if (btn_start.getText().toString().equals("开始保存数据")) {
                        start_save = true;
                        btn_start.setText("停止保存数据");

                        Time t = new Time(); // or Time t=new Time("GMT+8"); 加上Time Zone资料。
                        t.setToNow(); // 取得系统时间。
                        String str_time = t.year + "_" + t.month + "_" + t.monthDay + "_" +
                                t.hour + "_" + t.minute + "_" + t.second ;
                    ...
                    } else if (btn_start.getText().toString().equals("停止保存数据")) {
                        try {
                            outStream.close();
                        } catch (IOException e) {
                            e.printStackTrace();
        ...
        btn_startSensor.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (btn_startSensor.getText().toString().equals("开始采集数据")){
                    btn_startSensor.setText("停止采集数据");
                    startSensor();
                    ...
                    if (btn_startSensor.getText().toString().equals("停止采集数据")){

                        btn_startSensor.setText("开始采集数据");
                        stopSensor();
                        if(btn_start.getText().toString().equals("开始保存数据")){
                            btn_start.setEnabled(false);
                        }
